...
function init_wavesurfer() {
  {
    ...
    localforage.getItem(key_annotation, (err, data_annotation) => {
      ...
      is_annotated = data_annotation.length > 0;
    });
...

  {
    ...
    wavesurfer.on("waveform-ready", function() {
      ...
      const peak = wavesurfer.backend.getPeaks(num_subranges);
      const ready_region = extractRegions(
        peak,
        duration,
        unit_second
      )
...

  {
    ...
    const onChangeSpeed = function(e) {
      wavesurfer.setPlaybackRate(e.target.value);
    };
    speedInput.addEventListener("input", onChangeSpeed);
    speedInput.addEventListener("change", onChangeSpeed);
  }
